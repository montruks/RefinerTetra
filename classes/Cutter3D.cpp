...
namespace GeDiM
{
	...
	bool CutterByDomains::CompareByPlane(const GenericDomain* first, const GenericDomain* second)
	{
		...
		Vector3d tangentVectorEdgeSecond = second->Vertex(1) - second->Vertex(0); /*Vettore tangente secondo lato*/
	...
	const Output::ExitCodes CutterMesh3D::CutFace(GenericFace& face, const vector<const GenericPoint*>& domainPointsInFace, const double& toll)
	{
		...
		key << "TestDomain";
		face.SetState(false);
		face.InitializeChilds(2); /*riserva spazio di memoria per due facce figlie*/

		GenericFace& newFace_1 = *(mesh->CreateFace()); /*crea una faccia e la associa alla mesh*/
		mesh->AddFace(&newFace_1);
		face.AddChild(&newFace_1); /*newFace_1 diventa figlia di Face*/
		/// definisco proprietà a newFace_1
		newFace_1.SetFather(&face);
		newFace_1.InheritPropertiesByFather();
		///Crea le due celle che hanno in comune quella faccia
        ...
        ///Inserisce punti e lati alle facce generate
